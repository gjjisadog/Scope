...
**Tech Stack:** Rust 2021, eframe/egui 0.27, custom PNG/SVG canvas, new OpenXML `.docx` writer using `zip`, existing Rust unit tests and manual UI verification.

---

## File Map

- Modify `Cargo.toml`: bump version and add `zip` dependency.
- Modify `Cargo.lock`: generated by `cargo check` after dependency/version changes.
...
## Task 1: Dependencies, Versioning, And Module Registration
...
- [ ] **Step 1: Update Cargo metadata and dependency**
...
Expected: dependencies resolve and `Cargo.lock` updates without compiling the app.
...
fn write_word_report_to_writer<W: Write + std::io::Seek>(
    writer: W,
    report: &WordReport,
    ...
    zip.write_all(content_types(report).as_bytes())?;
    zip.start_file("_rels/.rels", options)?;
    zip.write_all(root_rels().as_bytes())?;
    zip.start_file("word/_rels/document.xml.rels", options)?;
    zip.write_all(document_rels(report).as_bytes())?;
    zip.start_file("word/document.xml", options)?;
    zip.write_all(document_xml(report).as_bytes())?;

    for (index, figure) in report.figures.iter().enumerate() {
        zip.start_file(format!("word/media/image{}.png", index + 1), options)?;
        zip.write_all(&figure.png)?;
    }
...
- [ ] **Step 5: Add XML element helpers**
...
mod tests {
    ...
    fn word_report_contains_required_parts_and_image_relationship() {
        ...
        assert!(text.contains("[Content_Types].xml") || bytes.starts_with(b"PK"));
...
Expected: app compiles with new modules and dependency.
