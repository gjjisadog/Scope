...
0.15.2 加固 SCP1 V2：独立 `logical_cycle_sequence` 不再复用 stream-local `row_sequence`；有界因果缓存支持乱序延迟匹配；Capture 失败与终态释放缓存但保持 V2 会话；row/timestamp 严格对应，心跳支持多 nonce 窗口。默认 GUI、SCP1 V1 和 `.scope V1` 保持不变。详细规范见 [SCP1 V2](docs/protocols/scp1-live-scope-v2.md)。
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/windows-acceptance.ps1 `
  -MsiPath dist\ScopeAnalyzer-0.15.2-win-x64.msi `
  -ZipPath dist\ScopeAnalyzer-0.15.2-win-x64.zip `
...
msiexec.exe /i dist\ScopeAnalyzer-0.15.2-win-x64.msi /qn /norestart
...
# 验收结束后由管理员清理部署
msiexec.exe /x dist\ScopeAnalyzer-0.15.2-win-x64.msi /qn /norestart
```
...
- `dist/ScopeAnalyzer-0.15.2-win-x64.zip`
- `dist/ScopeAnalyzer-0.15.2-win-x64.msi`
