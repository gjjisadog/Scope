...
变量显示名、通道显示状态、FFT 通道、采样频率、滚轮缩放敏感度和界面语言可通过顶部 `Import Config` / `Export Config` 导入导出。

采样频率可在 `Options` 中设置，默认 `1000 Hz`；云端 `Content` CSV 会用该频率生成秒级时间轴，FFT 也使用该设置。

界面语言可在 `Options` 中选择 `中文` 或 `English`。
...
- `Options`：可调整鼠标滚轮缩放敏感度，并可切换系统语言 `中文` / `English`。
