...
0.15.1 加固 SCP1 V2：`CausalRelation` 参与跨 Stream 逻辑控制周期校验；Capture 在到包时校验描述符、连续行、CRC32C、总数及内存上限；V2 具备双向心跳、STATUS/ERROR、主机背压统计和固定采样周期校验。默认 GUI、SCP1 V1 和 `.scope V1` 保持不变。详细规范见 [SCP1 V2](docs/protocols/scp1-live-scope-v2.md)。
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/windows-acceptance.ps1 `
  -MsiPath dist\ScopeAnalyzer-0.15.1-win-x64.msi `
  -ZipPath dist\ScopeAnalyzer-0.15.1-win-x64.zip `
...
msiexec.exe /i dist\ScopeAnalyzer-0.15.1-win-x64.msi /qn /norestart
...
# 验收结束后由管理员清理部署
msiexec.exe /x dist\ScopeAnalyzer-0.15.1-win-x64.msi /qn /norestart
```
...
- `dist/ScopeAnalyzer-0.15.1-win-x64.zip`
- `dist/ScopeAnalyzer-0.15.1-win-x64.msi`
